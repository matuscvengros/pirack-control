...
export async function getData(_config: ModuleConfig): Promise<ModuleData> {
	...
	return { on: anyOn, relays: relayStates, gpioAvailable: await isGpioAvailable() };
}
...
export async function onAction(
	...
	if (action === 'toggle') {
		const currentStates = await getRelayStates();
		const anyOn = currentStates.some((s) => s);
		const newStates = await setAllRelays(!anyOn);
